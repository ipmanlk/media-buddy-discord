...
const getVidInfo = (link) => {
    return new Promise((resolve, reject) => {
        const url = `http://107.173.126.17/youtube/info.php?url=${Utf8.encode(link)}`;
        try {
...
const getVidDownLink = (link, format) => {
    return new Promise((resolve, reject) => {
        ...
        const url = `http://107.173.126.17/youtube/download.php?url=${Utf8.encode(link)}&code=${code}`;
